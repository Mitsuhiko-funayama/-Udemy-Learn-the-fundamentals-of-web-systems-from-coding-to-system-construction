...
function tenko($student, $is_absent = false) {
    if ($is_absent) {
        // $is_absent が true（欠席）のとき // if文で最初に入るのは正しい時（true）
        echo $student . "は欠席しています。";
    } else {
        // $is_absent が false（出席）のとき // if文で後に入るのは間違いの時（false）
...
// --------------------------------------------------------------------------
// [自分の回答]
$num = 1; // グローバルスコープで定義された $num 仮に引数を1とする
$step = 1; // $stepのデフォルト引数
function counter($num + $step) -> $num { // カウンター関数を定義
        echo $num; // 出力
}
//----------------------------------------------------------------------------
// [AIの回答]
